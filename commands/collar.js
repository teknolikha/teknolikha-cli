...
module.exports = function (program) {
  const command = new Command("collar");

  command
    .description(
      "Create 18.5x7.5 inch landscape collar images with HEX color and convert to the default ICC profile",
    )
    ...
    .option(
      "-o, --output <folder>",
      "Output folder. Default: Desktop/collars/[PROJECT]/[HEX]",
    )
  ...
  function getDefaultOutputDirectory(project, hex) {
    return path.join(os.homedir(), "Desktop", "collars", project, hex);
  }
  ...
  command.action(async (options) => {
    console.log("");
    console.log("========================================");
    console.log(" TeknoLikha Collar Generator");
    console.log("========================================");
    console.log("");
    ...
    if (!/^[0-9A-F]{6}$/.test(hex)) {
      ...
      console.error(
        'Example: teknolikha collar --project "Project Alpha" --hex "#FF0000"',
      );
    ...
    if (!Number.isInteger(copy) || copy < 1) {
      ...
      console.error(
        'Example: teknolikha collar --project "Project Alpha" --hex "#000000" --copy 5',
      );
    ...
    for (let i = 1; i <= copy; i++) {
      const baseName = `collar_18.5x7.5_${hex}`;
