...
module.exports = function (program) {
  ...
  command
    ...
    .option("-i, --icc <name>", "ICC profile name (default: 3PASS TARP)")
    ...
    .option("--list-icc", "Display available ICC/ICM profiles");
  ...
  command.action(async (image, options) => {
      ...
      return new Promise((resolve) => {
        execFile("magick", ["-version"], (error, stdout) => {
          if (error) {
            ...
            console.error("");
            console.error("Please make sure ImageMagick is installed");

            console.error("and the 'magick' command is available.");
          ...
          console.log("");
          console.log("ImageMagick :", firstLine);
    ...
    if (options.listIcc) {
      console.log("");
      console.log("ICC Profile Directory:");
      ...
      if (profiles.length === 0) {
        console.log("No ICC/ICM profiles found.");
    ...
    const profileName = options.icc || "3PASS TARP";

    const destinationProfile = findICCProfile(profileName);

    if (!destinationProfile) {
      console.error("");
      console.error(`❌ ICC profile not found: ${profileName}`);

      console.log("");

      console.log("Run:");

      console.log("  teknolikha iccconvert --list-icc");
    ...
    console.log("Source        :", image ? image : "Current directory");

    console.log("ICC Profile   :", path.basename(destinationProfile));
      ...
      return new Promise((resolve) => {
        ...
        execFile(
          ...
          (profileError, profileOutput) => {
            ...
            console.log("  Destination:", path.basename(destinationProfile));
            ...
            if (!hasProfile) {
              if (!sRGBProfile) {
                console.error("  ❌ sRGB.icc not found in profiles folder.");

                console.error("  Add an sRGB ICC profile to:");
            ...
            execFile(
              ...
              (error, stdout, stderr) => {
                ...
                execFile(
                  ...
                  ["identify", "-format", "%[profile:icc]", outputFile],

                  (verifyError, verifyOutput) => {
                    if (verifyError || !verifyOutput || !verifyOutput.trim()) {
                      console.error(
                    ...
                    console.log("  ✓ ICC profile embedded");
